"""
Resnet from fastreid
https://github.com/DengpanFu/LUPerson/blob/main/fast-reid/fastreid/modeling/backbones/resnet.py#L15
"""
...
class ResNet(nn.Module):
    def __init__(self, num_classes, loss, last_stride, bn_norm, with_ibn, with_se, with_nl, block, layers, non_layers):
...
def init_pretrained_weights(model, url):
    pass

def lup_resnet50(num_classes, loss='softmax', pretrained=True, **kwargs):
    ...
    model = ResNet(
        ...
        non_layers=nl_layers_per_stage
    )

    if pretrained:
        init_pretrained_weights(model, model_urls['lup_r50'])

    return model

def lup_resnet100(num_classes, loss='softmax', pretrained=True, **kwargs):
    num_blocks_per_stage = [3, 4, 23, 3]
    nl_layers_per_stage = [0, 2, 9, 0]
    block =  Bottleneck
    pass

def lup_resnet152(num_classes, loss='softmax', pretrained=True, **kwargs):
    num_blocks_per_stage = [3, 8, 36, 3]
    nl_layers_per_stage = [0, 4, 12, 0]
    block =  Bottleneck
    pass